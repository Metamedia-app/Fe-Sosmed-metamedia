...
import { getGroupMessages, sendGroupMessage, sendGroupTypingStatus, deleteGroupMessage } from '@/utils/chatMatkul';
import { ArrowLeft, Send, Paperclip, Check, CheckCheck, Smile, Camera } from 'lucide-react-native';
...
export default function GroupChatRoomScreen() {
  ...
  const performDeleteMessage = async (messageId: string) => {
    if (!token) return;
    const result = await deleteGroupMessage(messageId, token);
    if (result.success) {
  ...
  const handleDeleteMessage = (message: any) => {
    ...
    const options: any[] = [
      { 
        text: "Hapus / Tarik Pesan", 
        style: "destructive",
        onPress: () => performDeleteMessage(message._id)
      },
    ...
    <>
      ...
      <View style={[styles.header, { backgroundColor: theme.card, shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.05, shadowRadius: 5, elevation: 3 }]}>
        ...
        <View style={styles.headerAvatarPlaceholder}>
          <Text style={{ color: '#FFF', fontWeight: 'bold', fontSize: 16 }}>{groupName ? groupName.charAt(0).toUpperCase() : 'G'}</Text>
        </View>
        <View style={styles.headerInfo}>
          ...
          <Text style={[styles.headerStatus, { color: remoteTyping ? theme.tint : theme.description }]}>
            {remoteTyping ? 'Seseorang sedang mengetik...' : 'Online'}
          </Text>
        </View>
      </View>
