...
import { getFollowing, FollowUser, followUser, unfollowUser, getFollowers, getOtherUserProfile } from '@/utils/follow';
...
export default function FollowingScreen() {
  ...
  const fetchUsers = async () => {
    ...
    setIsLoading(true);
    try {
      const [followingRes, followersRes, targetUserRes] = await Promise.all([
      ...
      if (followingRes.success) {
        ...
        // Initialize following states - since this is following list, everyone is followed
        const states: Record<string, boolean> = {};
        followingList.forEach((u: FollowUser) => {
          states[u._id] = true;
        });
        ...
        const followersCount = followersRes.success ? followersRes.data.followers.length : 0;
        
        // Friends count
        let friendsCount = 0;
        if (followersRes.success) {
          const followersIds = new Set(followersRes.data.followers.map((u: any) => u._id));
          friendsCount = followingList.filter((u: any) => followersIds.has(u._id)).length;
        }
      ...
      >
        {loadingStates[item._id] ? (
          ...
          <Text style={[
            ...
            {followingStates[item._id] ? 'Mengikuti' : 'Ikuti'}
          </Text>
        )}
      </TouchableOpacity>
    </TouchableOpacity>
  ...
  return (
    <FollowListLayout
      ...
      onRefresh={onRefresh}
      onTabChange={(tab) => {
        if (tab === 'followers') router.replace(`/user/${id}/followers`);
